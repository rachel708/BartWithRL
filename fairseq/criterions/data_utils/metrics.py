...
def compute_auc(predicts, labels, pr_file = "/home/work/tianqi/paraphrase/mtdnn/mt-dnn/scripts/checkpoints/pr/pr.tsv"):
    auc = roc_auc_score(labels, predicts)
    precision, recall, thresholds = precision_recall_curve(labels, predicts)
    with open(pr_file, 'w', encoding='utf-8') as writer:
        for i, p in enumerate(precision):
            if i == len(precision)-1:
                break
            r = recall[i]
            t = thresholds[i]
            writer.write(str(p) + "\t" + str(r) + "\t" + str(t) + "\n")
    return 100.0 * auc
